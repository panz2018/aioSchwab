...
class Tokens:
    def __init__(
        ...
        self._validate_input(
            app_key, app_secret, callback_url, tokens_file, call_on_notify
        )
        ...
        self._capture_callback = (
            capture_callback  # use a webserver with self-signed cert to callback
        )
    ...
    def update_tokens(self, force_access_token=False, force_refresh_token=False):
        ...
        if 30 <= rt_delta <= 43300 and rt_delta % 900 <= 30:  # call every hour
            ...
            if rt_delta < 21700 and rt_delta % 3600 <= 30:
                call_notifier(
                    message=f"Refresh token expires in less than {int(abs(rt_delta) / 3600):01} hours",
                    importance=int(abs(rt_delta) / 3600),
                )
        ...
        # check if we need to update access token
            ...
            self._client.logger.info(
                "The access token has expired, updating automatically."
            )
    ...
    def update_access_token(self):
        ...
        if response.ok:
            ...
            self._client.logger.info(
                f"Access token updated: {self._access_token_issued}"
            )
        else:
            self._client.logger.error(response.text)
            self._client.logger.error(
                f"Could not get new access token; refresh_token likely invalid."
            )
    ...
    def _generate_certificate(
        ...
        builder = builder.subject_name(
            x509.Name(
                [
                    ...
                    x509.NameAttribute(
                        NameOID.ORGANIZATIONAL_UNIT_NAME, "Authentication"
                    ),
                ]
            )
        )
        ...
        self._client.logger.info(
            f"Certificate generated and saved to {key_filepath} and {cert_filepath}"
        )
    ...
    def update_refresh_token(self):
        ...
        if (
            self._capture_callback and not url_port.isdigit()
        ):  # if there is a port then capture the callback url
            self._client.logger.error(
        ...
        if self._capture_callback:
            self._update_refresh_token_from_code(
                self._launch_capture_server(url_base, int(url_port))
            )
        else:
            response_url = input(
