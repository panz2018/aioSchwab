...
class Stream:
    ...
    async def _start_streamer(
        ...
        while True:
            try:
            ...
            ) as e:  # "received 1000 (OK); then sent 1000 (OK)"
                self.active = False
                ...
                if (
                    ...
                    self._client.logger.warning(
                        f"Stream has crashed within 90 seconds, likely no subscriptions, invalid login, or lost connection (not restarting)."
                    )
                    break
        ...
        try:
            ...
            if parameters is not None and service is not None:
                ...
                if command == "ADD":
                    ...
                    command == "VIEW"
                ):  # not sure if this is even working on Schwab's end :/
                    for key in self.subscriptions[service].keys():
                        self.subscriptions[service][key] = fields
        except Exception as e:
            self._client.logger.error(e)
            self._client.logger.error(
                "Error recording request - subscription not saved."
            )
